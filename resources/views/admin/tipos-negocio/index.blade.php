...
@section('content_header_extra')
    <div class="d-flex flex-wrap align-items-start justify-content-between">
        <div>
            <h1 class="mb-1">Tipos de negocio</h1>
            <p class="text-muted mb-0">Gestiona el catálogo base de tipos de negocio.</p>
        </div>

        <div class="mt-3 mt-md-0">
            <a href="{{ route('admin.tipos-negocio.create') }}" class="btn btn-primary">
...
@section('content_body')
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body border-bottom">
            <form method="GET" action="{{ route('admin.tipos-negocio.index') }}" class="js-basic-validation-form">
                <div class="form-row align-items-end">
                    ...
                    <div class="form-group col-md-12 col-lg-3 mb-0 d-flex justify-content-lg-end mt-3 mt-lg-0">
                        ...
                        <a href="{{ route('admin.tipos-negocio.index') }}" class="btn btn-light border">
                            Limpiar
                        </a>
                    </div>
                </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    @forelse($tiposNegocio as $tipoNegocio)
                        <tr data-row-id="{{ $tipoNegocio->id }}">
                                ...
                                <div class="js-inline-wrapper">
                                    <div class="d-flex align-items-start justify-content-between">
                                        ...
                                        <button
                                            ...
                                            data-target="inline-form-{{ $tipoNegocio->id }}"
                                        >
                                    ...
                                    <form
                                        id="inline-form-{{ $tipoNegocio->id }}"
                                        action="{{ route('admin.tipos-negocio.inline-update', $tipoNegocio) }}"
                                        method="POST"
                            ...
                            <td class="text-muted align-top">
                                {{ $tipoNegocio->descripcion ? \Illuminate\Support\Str::limit($tipoNegocio->descripcion, 90) : 'Sin descripción' }}
                            </td>
                            <td class="text-center align-top">
                                <span class="badge badge-light border">{{ $tipoNegocio->negocios_count }}</span>
                            </td>
                            <td class="text-nowrap text-muted align-top">
                                {{ optional($tipoNegocio->updated_at)->format('d/m/Y H:i') }}
                            </td>
                            <td class="text-right text-nowrap align-top">
                                <a href="{{ route('admin.tipos-negocio.show', $tipoNegocio) }}" class="btn btn-xs btn-light border">
                                    Ver
                                </a>
                                <a href="{{ route('admin.tipos-negocio.edit', $tipoNegocio) }}" class="btn btn-xs btn-light border">
                                    Editar
                                </a>
                                <button
                                    type="button"
                                    class="btn btn-xs btn-outline-danger js-delete-button"
                                    data-action="{{ route('admin.tipos-negocio.destroy', $tipoNegocio) }}"
                                    data-message="Vas a eliminar el tipo de negocio &quot;{{ e($tipoNegocio->nombre) }}&quot;."
                                >
                                    Eliminar
                                </button>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-5 text-muted">
                                No hay tipos de negocio registrados con los filtros actuales.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($tiposNegocio->hasPages())
            <div class="card-footer bg-white">
                {{ $tiposNegocio->links('pagination::bootstrap-4') }}
            </div>
        @endif
    </div>
